...
public class ActionPropertiesDeserializer extends JsonDeserializer<ActionProperties> {
    ...
    @Override
    public ActionProperties deserialize(JsonParser p, DeserializationContext ctxt)
    ...
    public ActionProperties deserialize(JsonNode jsonNode) {
        if (jsonNode instanceof ObjectNode) {
            ...
            if (isProjectile) {
                ...
                return new ActionProperties(actionName, gameCharName, actionPriority, duration, hitBoxStartTime,
                        isSelfInterruptible, isEnemyInterruptible, totalFrames, isAnimationLoop, animationLoopStartTime,
                        isMovement, knockBack, hitbox, damage, isProjectile);
            }

            if (hasHitbox && hasKnockback) {
                return new ActionProperties(actionName, actionPriority, duration, hitBoxStartTime, isSelfInterruptible,
                        isEnemyInterruptible, totalFrames, isAnimationLoop, animationLoopStartTime, isMovement,
                        knockBack, hitbox, damage);
            }

            if (hasKnockback) {
                return new ActionProperties(actionName, actionPriority, duration, isSelfInterruptible,
                        isEnemyInterruptible, totalFrames, isAnimationLoop, animationLoopStartTime, isMovement,
                        knockBack);
            }

            // no knockback no hitbox
            ActionProperties actionProperties = new ActionProperties(actionName, actionPriority, duration,
                    isSelfInterruptible, isEnemyInterruptible, totalFrames, isAnimationLoop, animationLoopStartTime,
                    isMovement);
